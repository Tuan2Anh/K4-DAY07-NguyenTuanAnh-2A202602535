...
class SentenceChunker:
    ...
    def chunk(self, text: str) -> list[str]:
        ...
        # Split after sentence boundary (. ! ?) followed by whitespace or newline
        # Using positive lookbehind preserves punctuation at the end of sentences
        pattern = r"(?<=[.!?])(?:\s+|\n+)"
...
class RecursiveChunker:
    ...
    def _split(self, current_text: str, remaining_separators: list[str]) -> list[str]:
        ...
        if not remaining_separators:
            # Fallback when no separators left: hard slice by chunk_size
        ...
        for part in splits:
            if len(part) > self.chunk_size:
                processed_pieces.extend(self._split(part, next_seps))
            else:
                processed_pieces.append(part)

        # Merge adjacent pieces to avoid tiny fragments
        chunks: list[str] = []
        current_chunk = ""
